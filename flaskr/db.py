...
def get_db():
    ...
    store the connection in g, to be reused."""

    if 'db' not in g:
        # create a Connection object that represents the database
        g.db = sqlite3.connect(
            # current_app is the proxy to the app object
            # as it is created in a factory
...
def init_db():
    ...
    # use with to close the connection afterwards
    with current_app.open_resource('schema.sql') as f:
        # create cursor and execute script to initiate database
        db.executescript(f.read().decode('utf8'))


# Use the click command line to initiate the database,
# Can be called using: flask init-db
...
def init_app(app):
    """ Registers the close_db and init_db_command functions with the app
    Given the app is created with a factory function,
    and the app instance isn't available """
